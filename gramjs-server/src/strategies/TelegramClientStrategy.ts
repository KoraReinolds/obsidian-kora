...
class TelegramClientStrategy {
	 ...
	 */
	private processLinkPreview(
		targetOptions: any,
		sourceOptions: MessageOptions
	): void {
	...
	private processReplyMarkup(
		...
		if (sourceOptions.replyMarkup) {
			...
			if (sourceOptions.replyMarkup.inline_keyboard) {
				...
				targetOptions.buttons = sourceOptions.replyMarkup.inline_keyboard.map(
					(row: any) =>
						row.map((btn: any) => ({
							text: btn.text,
							...(btn.url ? { url: btn.url } : {}),
	...
	sanitizeResult(result: any): any {
		...
		const cleanResult = {
			...
			editDate: result.editDate
				? new Date(result.editDate * 1000).toISOString()
				: null,
