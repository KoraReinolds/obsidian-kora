...
import type { TelegramClientStrategy, TelegramConfig } from './TelegramClientStrategy.js';
...
class StrategyFactory {
   ...
   */
  static createStrategy(mode: string, config: TelegramConfig): TelegramClientStrategy {
   ...
   */
  static createValidatedStrategy(mode: string, config: TelegramConfig): TelegramClientStrategy {
   ...
   */
  static validateConfigCompleteness(mode: string, config: TelegramConfig): ValidationResult {
    const required = this.getRequiredConfig(mode);
    const missing = required.filter(field => !config[field as keyof TelegramConfig]);
