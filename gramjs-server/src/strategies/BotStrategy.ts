...
class BotStrategy extends TelegramClientStrategy {
	...
	validateConfig(): void {
		if (!this.config.botToken) {
			throw new Error(
				'Bot token is required for bot mode. Set TELEGRAM_BOT_TOKEN environment variable.'
			);
		}
	}
	...
	async getDialogsImpl(options: DialogOptions = {}): Promise<any[]> {
		try {
			...
			console.warn(
				'[BotStrategy] Bot dialog access limited: node-telegram-bot-api does not support getDialogs'
			);
	...
	async sendMessage(peer: string, options: MessageOptions): Promise<any> {
		try {
			console.log(
				`[BotStrategy] Sending message to peer: ${peer}, ${JSON.stringify(options)}`
			);
			...
			if (options.formattingEntities && options.formattingEntities.length > 0) {
				...
				console.log(
					`[BotStrategy] Sending with formattingEntities:`,
			...
			if (error.message.includes('Bad Request: chat not found')) {
				throw new Error(
					'Bot cannot send to this peer. Make sure bot is added to the chat and has permission to send messages.'
				);
			}
			if (error.message.includes('Forbidden')) {
				throw new Error(
					"Bot is blocked or doesn't have permission to send messages to this chat."
				);
			}
	...
	async editTgMessage(
		...
		try {
			console.log(
				`[BotStrategy] Editing message ${messageId} in peer: ${peer}`
			);
			...
			if (options.replyMarkup && options.replyMarkup.inline_keyboard) {
				opts.reply_markup = options.replyMarkup;
				console.log(
					`[BotStrategy] Editing with replyMarkup:`,
					JSON.stringify(opts.reply_markup)
				);
			}
			...
			if (options.formattingEntities && options.formattingEntities.length > 0) {
				opts.entities = options.formattingEntities;
				console.log(
					`[BotStrategy] Editing with formattingEntities:`,
					options.formattingEntities.length,
					'entities'
				);
			}
			...
			if (error.message.includes('Forbidden')) {
				throw new Error(
					"Bot doesn't have permission to edit messages in this chat."
				);
			}
	...
	async getMessages(peer: string, options: any = {}): Promise<any[]> {
		try {
			console.log(
				`[BotStrategy] Getting messages from peer: ${peer} (bot mode)`
			);
			console.warn(
				'[BotStrategy] Bot message access limited: node-telegram-bot-api does not support getMessages'
			);
			...
			if (error.message.includes('CHAT_ADMIN_REQUIRED')) {
				throw new Error(
					'Bot needs admin rights to read messages in this chat.'
				);
			}
	...
	async sendFile(peer: string, options: FileOptions): Promise<any> {
		try {
			...
			if (error.message.includes('Bad Request: chat not found')) {
				throw new Error(
					'Bot cannot send files to this peer. Make sure bot is added to the chat.'
				);
			}
			if (error.message.includes('Forbidden')) {
				throw new Error(
					"Bot is blocked or doesn't have permission to send files to this chat."
				);
			}
