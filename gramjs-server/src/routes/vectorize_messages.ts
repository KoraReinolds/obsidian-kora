	...
	app.post('/vectorize_messages', async (req: Request, res: Response) => {
		try {
			...
			const contentList = filtered
				.filter(
					m =>
						(m as any).message && String((m as any).message).trim().length > 0
				)
				.map(m => ({
			...
			const results = await vectorService.batchVectorize(
				contentList,
				progress => {
					// eslint-disable-next-line no-console
					console.log(
						`[/vectorize_messages] Progress: ${progress.current}/${progress.total}`
					);
				}
			);
