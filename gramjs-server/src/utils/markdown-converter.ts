 ...
 */
export function convertToGramJSEntities(
	entities: any[] = []
...
export function processMessage(
	...
	const { message, entities, buttons, disableWebPagePreview, mode } = options;
	...
	if (message) {
		...
		if (finalMessage.length > 4096) {
			throw new Error(
				`Message too long: ${finalMessage.length} characters (max 4096)`
			);
		}
