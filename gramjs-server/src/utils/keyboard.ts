 ...
 */
export function createInlineKeyboard(buttons?: Array<Array<{ text: string; url?: string; data?: string }>>): any {
  ...
  const rows = buttons.map(row => {
    const buttonRow = row.map(btn => {
      ...
      if (btn.data) {
        return new Api.KeyboardButtonCallback({ text: btn.text, data: Buffer.from(btn.data, 'utf-8') });
      }
      return new Api.KeyboardButtonCallback({ text: btn.text, data: Buffer.from(btn.text, 'utf-8') });
    });
