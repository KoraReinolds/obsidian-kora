...
describe('MarkdownToTelegramConverter - Edge Cases', () => {
  ...
  describe('Performance Tests', () => {
    test('should handle large documents efficiently', () => {
      const largeContent = Array(1000).fill('**Bold** text with *italic* and `code`').join('\n');
    ...
    test('should handle deeply nested formatting', () => {
      const deeplyNested = '**Bold with *italic with `code with [spoiler]`* end**';
  ...
  describe('Unicode and Special Characters', () => {
    ...
    test('should handle multibyte characters', () => {
      ...
      expect(result.entities).toContainEqual({ type: 'italic', offset: 8, length: 7 });
    });

    test('should handle Chinese characters', () => {
      const result = converter.convert('**中文** 测试 *格式*');
      expect(result.text).toBe('中文 测试 格式');
      expect(result.entities).toContainEqual({ type: 'bold', offset: 0, length: 2 });
      expect(result.entities).toContainEqual({ type: 'italic', offset: 6, length: 2 });
    });
  ...
  describe('Malformed Markdown', () => {
    ...
    test('should handle mixed quotation marks', () => {
      const result = converter.convert('**"Mixed quotes"** and *\'single quotes\'*');
      expect(result.text).toBe('"Mixed quotes" and \'single quotes\'');
      expect(result.entities).toHaveLength(2);
    });

    test('should handle escaped characters', () => {
      const result = converter.convert('\\*\\*Not bold\\*\\* and \\[not spoiler\\]');
  ...
  describe('Boundary Conditions', () => {
    test('should handle formatting at text boundaries', () => {
      ...
      expect(result.entities[1]).toEqual({ type: 'bold', offset: 13, length: 3 });
    });
    ...
    test('should handle newlines in formatting', () => {
      ...
      expect(result.entities).toContainEqual({ type: 'bold', offset: 0, length: 17 });
    });
  });
  ...
  describe('Complex Blockquote Scenarios', () => {
    ...
    test('should handle nested blockquote-like syntax', () => {
      const result = converter.convert('>[! outer]-\n>> Inner quote\n>Normal line');
...
> Quote after`;
  ...
  describe('Memory and Resource Tests', () => {
    ...
    test('should handle alternating patterns', () => {
      const alternating = Array(100).fill(null)
...
Check [GitHub](https://github.com) and [Slack](https://slack.com) for updates.`;
      ...
      expect(result.entities.some(e => e.type === 'expandable_blockquote')).toBe(true);
