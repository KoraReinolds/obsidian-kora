...
expect.extend({
	toBeEntityType(received: any, expectedType: string) {
		...
		if (pass) {
			...
			return {
				message: () =>
					`expected entity to be of type ${expectedType}, but got ${received?.type || 'undefined'}`,
				pass: false,
			};
		}
	},
	...
	toHaveEntityAt(
		...
		const entity = received.find(
			e => e.offset === offset && e.length === length
		);
		...
		if (pass) {
			return {
				message: () =>
					`expected not to have entity at offset ${offset} with length ${length}${type ? ` of type ${type}` : ''}`,
				pass: true,
			};
		} else {
			return {
				message: () =>
					`expected to have entity at offset ${offset} with length ${length}${type ? ` of type ${type}` : ''}, but found: ${JSON.stringify(received)}`,
				pass: false,
			};
		}
	},
