...
describe('MarkdownToTelegramConverter - Performance Tests', () => {
  ...
  describe('Benchmarks', () => {
    ...
    test('should handle medium content efficiently', () => {
      const content = Array(100).fill('**Bold** *italic* `code` [spoiler] text').join('\n');
    ...
    test('should handle large content within time limit', () => {
      const content = Array(1000).fill('**Bold** text with *formatting*').join('\n');
...
\`\`\``;
    ...
    test('should handle documentation with many code blocks', () => {
      const documentation = Array(20).fill(`
### API Endpoint
...
>This endpoint requires **authentication** via [API key]
  ...
  describe('Edge Case Performance', () => {
    ...
    test('should handle very long blockquotes', () => {
      const longLines = Array(100).fill('>Long line with **bold** and *italic* text').join('\n');
...
\`\`\`
  ...
  describe('Regression Tests', () => {
    test('should maintain performance with unicode content', () => {
      const unicodeContent = Array(500).fill('**粗体** *斜体* `代码` [剧透] 🎉').join('\n');
  ...
  describe('Comparative Performance', () => {
    test('should perform consistently across different content types', () => {
      const testCases = [
        ...
        { name: 'Mixed inline', content: Array(500).fill('**bold** *italic* `code`').join(' ') },
        { name: 'Code blocks', content: Array(100).fill('```\ncode\n```').join('\n') },
        { name: 'Blockquotes', content: Array(1000).fill('>quote line').join('\n') },
      ];
