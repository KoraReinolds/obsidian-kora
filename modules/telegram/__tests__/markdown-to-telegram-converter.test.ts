...
\`\`\``;
	...
	describe('Links', () => {
		...
		test('should remove links when preserveLinks is false', () => {
			const result = converter.convert('Visit [Google](https://google.com)', {
				preserveLinks: false,
			});
		...
		test('should handle multiple links', () => {
			const result = converter.convert(
				'[First](url1) and [second](url2) links'
			);
...
text with [spoiler] here`;
		...
		test('should handle mixed formatting outside blockquotes', () => {
			const result = converter.convert(
				'**Bold** *italic* `code` [spoiler] and ~~strike~~'
			);
...
**Bold text** in section`;
		...
		test('should preserve formatting order and nesting', () => {
			...
			expect(italicEntity?.offset + italicEntity?.length).toBeLessThanOrEqual(
				32
			);
	...
	describe('Validation', () => {
		test('should validate text length', () => {
			...
			expect(validation.issues).toContain(
				'Text too long: 5000 characters (max 4096)'
			);
		});
...
**Bold** text`;
	...
	describe('Entity Sorting and Positioning', () => {
		...
		test('should handle overlapping entities correctly', () => {
			...
			expect(italicEntity?.offset).toBeGreaterThanOrEqual(
				boldEntity?.offset || 0
			);
			expect(
				(italicEntity?.offset || 0) + (italicEntity?.length || 0)
			).toBeLessThanOrEqual(
				(boldEntity?.offset || 0) + (boldEntity?.length || 0)
			);
		});
	});
});
