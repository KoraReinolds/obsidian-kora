...
export function splitLongParagraphs(blocks: ParsedBlock[], options: Required<ChunkOptions>): ParsedBlock[] {
  ...
  for (const b of blocks) {
    if (b.type !== 'paragraph') { result.push(b); continue; }
    const words = b.text.split(/\s+/).length;
    const tokens = roughTokenCount(b.text);
    if (words <= (options.longParagraphWordThreshold || 300) && tokens <= 800) { result.push(b); continue; }

    const sentences = splitIntoSentences(b.text);
    const overlap = 2;
    let start = 0;
    while (start < sentences.length) {
      const end = Math.min(start + Math.ceil(sentences.length / 2), sentences.length);
