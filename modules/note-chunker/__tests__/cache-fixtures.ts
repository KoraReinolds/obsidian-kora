...
export function makeCacheFromMarkdown(md: string): CachedMetadataLike {
  ...
  while (i < lines.length) {
    ...
    if (fence) {
      ...
      let k = i + 1; let endOffset = at + line.length;
      while (k < lines.length) {
        if (new RegExp(`^(${fenceStr})`).test(lines[k])) { endOffset = starts[k] + lines[k].length; i = k + 1; break; }
        k++;
      }
    ...
    if (/^>\s?/.test(line)) {
      const start = { line: i, offset: at };
      let k = i + 1; while (k < lines.length && /^>\s?/.test(lines[k])) k++;
      const end = { line: k - 1, offset: starts[k - 1] + lines[k - 1].length };
    ...
    if (lm) {
      const start = { line: i, offset: at };
      let k = i; do {
        const l = lines[k]; if (!l || !/^(\s*)([-*+]\s+|\d+\.[)\s]+)/.test(l)) break;
    ...
    if (line.trim().length > 0) {
      const start = { line: i, offset: at };
      let k = i + 1; while (k < lines.length && lines[k].trim().length > 0) k++;
      const end = { line: k - 1, offset: starts[k - 1] + lines[k - 1].length };
