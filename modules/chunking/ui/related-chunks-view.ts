...
export class RelatedChunksView extends ItemView {
	...
	private renderRelatedChunkList(
		...
		for (const chunk of relatedChunks) {
			...
			item.style.cssText =
				'border:1px solid var(--background-modifier-border);border-radius:6px;padding:8px;background:var(--background-secondary);transition:background-color .15s,border-color .15s;cursor:pointer;';
			...
			badge.style.cssText =
				'font-size:10px;border-radius:8px;padding:2px 6px;vertical-align:middle;border:1px solid var(--background-modifier-border);background:var(--background-modifier-hover);color:var(--text-muted);';
			...
			if (chunk.sourceFile) {
				const sourceInfo = item.createEl('div', {
	...
	private createWeightIndicator(score: number): HTMLElement {
      ...
      `;
			...
			if (i < filledDots) {
				...
				let color = '';
				if (score >= 0.8)
					color = 'var(--color-green, #059669)'; // Very strong
				else if (score >= 0.6)
					color = 'var(--color-yellow, #d97706)'; // Strong
				else if (score >= 0.4)
					color = 'var(--color-orange, #ea580c)'; // Medium
	...
	private async renderUnsyncedNotes(wrapper: HTMLElement): Promise<void> {
		try {
			...
			separator.style.cssText =
				'height:1px;background:var(--background-modifier-border);margin:16px 0;';
			...
			for (const file of filesToShow) {
				const item = list.createEl('div');
				item.style.cssText =
					'padding:6px 8px;border:1px solid var(--background-modifier-border);border-radius:4px;background:var(--background-secondary);cursor:pointer;transition:background-color .15s,border-color .15s;font-size:12px;';
			...
			if (unsyncedFiles.length > 20) {
				const moreText = wrapper.createEl('div', {
					text: `... and ${unsyncedFiles.length - 20} more files`,
				});
