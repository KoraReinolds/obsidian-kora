...
export interface McpEndpointDefinition<TInput = any, TOutput = any> {
	...
	mcpTool: (baseUrl: string, input: TInput) => Promise<{
		content: Array<{ type: 'text'; text: string }>;
	}>;
}

export abstract class BaseEndpoint<TInput = any, TOutput = any> implements McpEndpointDefinition<TInput, TOutput> {
	...
	async mcpTool(baseUrl: string, input: TInput): Promise<{
		content: Array<{ type: 'text'; text: string }>;
	}> {
