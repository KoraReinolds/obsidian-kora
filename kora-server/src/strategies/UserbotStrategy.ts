...
class UserbotStrategy extends TelegramClientStrategy {
	...
	validateConfig(): void {
		if (!this.config.stringSession) {
			throw new Error(
				'Session string is required for userbot mode. Set TELEGRAM_SESSION environment variable.'
			);
		}
		if (!this.config.apiId || !this.config.apiHash) {
			throw new Error(
				'API ID and Hash are required. Get them from https://my.telegram.org/apps'
			);
		}
	}
	...
	async initialize(): Promise<UserInfo> {
		...
		console.log(
			'[UserbotStrategy] Session string length:',
			this.config.stringSession?.length || 0
		);
		...
		console.log(
			`[UserbotStrategy] Userbot connected successfully: @${me.username} (${me.firstName} ${me.lastName})`
		);
	...
	async getDialogsImpl(options: DialogOptions = {}): Promise<any[]> {
		console.log(
			'[UserbotStrategy] Getting dialogs with full userbot access...'
		);
	...
	async sendMessage(peer: string, options: MessageOptions): Promise<any> {
		try {
			...
			console.log(
				'[UserbotStrategy] Message sent successfully in userbot mode'
			);
			return {
				...result,
				mode: 'userbot',
			...
			if (error.message.includes('PEER_ID_INVALID')) {
				throw new Error(
					'Invalid peer ID. Check if the chat/user exists and is accessible.'
				);
			}
			if (error.message.includes('USER_DEACTIVATED')) {
				throw new Error(
					'Your account has been deactivated. Cannot send messages.'
				);
			}
			if (error.message.includes('FLOOD_WAIT')) {
				...
				throw new Error(
					`Rate limited. Please wait ${seconds} seconds before sending another message.`
				);
			}
	...
	async editTgMessage(
		...
		try {
			console.log(
				`[UserbotStrategy] Editing message ${messageId} in peer: ${peer}`
			);

			const result = await super.editTgMessage(peer, messageId, options);

			console.log(
				'[UserbotStrategy] Message edited successfully in userbot mode'
			);
			return {
				...result,
				mode: 'userbot',
			...
			if (error.message.includes('USER_DEACTIVATED')) {
				throw new Error(
					'Your account has been deactivated. Cannot edit messages.'
				);
			}
			if (error.message.includes('FLOOD_WAIT')) {
				const match = error.message.match(/FLOOD_WAIT_(\d+)/);
				const seconds = match ? match[1] : 'unknown';
				throw new Error(
					`Rate limited. Please wait ${seconds} seconds before editing another message.`
				);
			}
	...
	async getMessages(peer: string, options: any = {}): Promise<any[]> {
		try {
			console.log(
				`[UserbotStrategy] Getting messages from peer: ${peer} (userbot mode)`
			);
			return await super.getMessages(peer, options);
		} catch (error) {
			console.error(
				'[UserbotStrategy] Userbot message retrieval error:',
				error
			);

			if (error.message.includes('CHAT_ADMIN_REQUIRED')) {
				throw new Error(
					'Admin rights required to read message history in this chat.'
				);
			}
	...
	async sendFile(peer: string, options: FileOptions): Promise<any> {
		try {
			...
			if (error.message.includes('PEER_ID_INVALID')) {
				throw new Error(
					'Invalid peer ID for file sending. Check if the chat/user exists.'
				);
			}
			if (error.message.includes('MEDIA_TOO_BIG')) {
				throw new Error(
					'File is too large. Consider compressing or splitting the file.'
				);
			}
