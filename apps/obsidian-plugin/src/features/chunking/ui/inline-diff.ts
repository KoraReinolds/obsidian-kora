...
export function renderInlineDiff(
	...
	for (const p of parts) {
		...
		if (p.type === 'equal') {
			...
			span.textContent = p.text;
			span.style.cssText =
				'background:rgba(16,185,129,0.20);color:var(--color-green,#059669);border-radius:4px;padding:0 2px;';
		} else {
			span.textContent = p.text;
			span.style.cssText =
				'background:rgba(239,68,68,0.20);color:var(--color-red,#dc2626);text-decoration:line-through;border-radius:4px;padding:0 2px;';
		}
